...
import com.hh.hhojbackendcommon.exception.BusinessException;
import com.hh.hhojbackendcommon.exception.ThrowUtils;
...
public class UserController {
    ...
    private UserQuestionVO getUserQuestionVO(HttpServletRequest request,Long questionId) {
        ...
        QueryWrapper<QuestionSubmit> submitQueryWrapper = new QueryWrapper<QuestionSubmit>().eq("userId", user.getId());
        if (questionId!=null){
            submitQueryWrapper.eq("questionId",questionId);
        }
        List<QuestionSubmitVO> userQuestionsList = questionFeignClient.list(submitQueryWrapper).stream().map(QuestionSubmitVO::objToVo).peek(questionSubmitVO -> {
            Question question = questionFeignClient.getOne(new QueryWrapper<Question>().eq("id", questionSubmitVO.getQuestionId()));
